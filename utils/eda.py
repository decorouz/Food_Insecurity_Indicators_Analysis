...
def compute_groupby(df: pd.DataFrame, col1: str, col2: str, weight_col: str="weight_final") -> pd.DataFrame:
    """
...
def plot_grouped_data(pivot_df_percentage: pd.DataFrame, 
                      kind="barh", 
                      figsize=(12, 6),
                      **kwargs) -> None:
...
def percentage_crp_prod_dif(df, start, end, n):
    ...
    # Melt the result
    results = pd.melt(results.reset_index(drop=True).iloc[[1]], var_name="difficulty_type", 
        value_name="value").sort_values("value",ascending=False)
...
def plot_diff_variables(crp_proddiff_df: pd.DataFrame, title:str) -> None:
    ...
    ax = sns.barplot(crp_proddiff_df, x="value", y="difficulty_type",)
